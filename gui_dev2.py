						...
						if i> datasource_num:
							...
							if text.strip().startswith('Protocol'):
								...
								if protocol_flag==0:		
									#print(footnote_sub_list)
									footnote_sub=footnote_sub.replace('\n'+paragraphlist[i-1],'').replace(paragraphlist[i-1],'').replace(text,'')
									footnote_list=[table_num[-2],footnote_sub,footnote_sub_list[0:i-1]]
									footnote.append(footnote_list)
									#print(footnote)
						...
						for j in range(col):		  
							...
							if j>0:
								...
								# if header_list[i][j]!=header_list[i][j-1]:	  
									...
									# p.add_run(cell_list)
								elif header_list[i][j][0]==header_list[i][j-1][0] and header_list[i][j][0].strip().upper()=='(N = XX)':
									if i>0:
