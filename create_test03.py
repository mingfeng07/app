    ...
    for n in range(row_header_cnt):
        ...
        for k in range(col_header_cnt+1):
            if k==0 and n>0:
                ...
                if header_list[n][k-2][0]==header_list[n][k-1][0] and header_list[n][k-1][0].strip()!="" and n==0: 
                    # print("merge cell value",header_list[n][k-1][0])
                    col_lt,col_rt=tab.cell(n,k-1),tab.cell(n,k)
                    col_lt.merge(col_rt)	
                    cell_merge_of_list.append(dict_value)
                elif header_list[n][k-2][0]==header_list[n][k-1][0] and header_list[n][k-1][0].strip()!="" and n>0:
                    # print("merge cell value",header_list[n][k-1][0])
...
for i in range(len(table_T)):
    ...
    if tbl_flag==1:
        ...
        # print('start,end,body:',start,end,body_lastline_cell_list)
        ...
        for m in range(len(footnote_style)):
            if len(footnote_style[m])>0:
                if len(footnote_style[m][1])>0 and footnote_style[m][0].startswith('BeiGene')==0 and footnote_style[m][0].startswith('Protocol BGB')==0: #检查footnote是否为空
                    # print('---------bullet_flag---------',bullet_flag)
                    # print('---------footnote------------',footnote_style[m][0])
                    if bullet_flag==1 and footnote_style[m][0].strip()!='':
                        footnote_style[m][1][0][0]='  ●  '+footnote_style[m][1][0][0]
                        # print('-------bullet text---------',footnote_style[m][1][0][0])
